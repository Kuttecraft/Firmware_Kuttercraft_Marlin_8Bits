...
#include "dogm_font_data_Kuttercraft_symbols.h"   // The Marlin special symbols
#define FONT_SPECIAL_NAME Kuttercraft_symbols
...
//bool solo_una_vez_error_temp = true;
int lcd_contrast;
...
static void lcd_implementation_init() {
  ...
  //para medir el tiempo //deberia tener guardado el ultimo valor de tiempo
  millis_t last_time = 0;
    ...
    if (show_bootscreen) {
      ...
      do {
        ...
        //u8g.drawStr(62,10,"KUTTERCRAFT");
        u8g.setPrintPos(62, 10);
        lcd_printPGM(PSTR("KUTTERCRAFT"));
        u8g.setFont(u8g_font_6x10);
        //u8g.drawStr(62,54,"Industria");
        u8g.setPrintPos(62, 54);
        lcd_printPGM(PSTR("Industria"));
        u8g.setFont(u8g_font_6x10);
        //u8g.drawStr(62,62,"Argentina");
        u8g.setPrintPos(62, 62);
        lcd_printPGM(PSTR("Argentina"));
        u8g.setFont(u8g_font_5x8);
        //u8g.drawStr(62,20,"Configuracion");
        u8g.setPrintPos(62, 20);
        lcd_printPGM(PSTR("Configuracion"));
        u8g.setFont(u8g_font_5x8);
        //u8g.drawStr(62,29,"Vercion:");
        u8g.setPrintPos(62, 29);
        lcd_printPGM(PSTR("Version:"));
        u8g.setFont(u8g_font_5x8);
        u8g.drawStr(62,36,SHORT_BUILD_VERSION);
        u8g.drawStr(62,44,"Modelo " MSG_TIPO);
        u8g.setPrintPos(62, 44);
      } while (u8g.nextPage());
      delay(1000);//lcd_print
    }
...
FORCE_INLINE void _draw_heater_status(int x, int heater) {
  ...
  #else
    u8g.setPrintPos(68, 17);

    lcd_print(itostr3(int(degTargetHotend(0)) + 0.5 ));
    lcd_printPGM(PSTR(LCD_STR_DEGREE " "));
    u8g.setPrintPos(99, 17);
    lcd_print(itostr3(int((degTargetBed()) + 0.5)));
    lcd_printPGM(PSTR(LCD_STR_DEGREE " "));
    u8g.setPrintPos(68, 39);
    lcd_print(itostr3(int(degHotend(0) + 0.5)));
    lcd_printPGM(PSTR(LCD_STR_DEGREE " "));
    u8g.setPrintPos(99, 39);
    lcd_print(itostr3(int(degBed()) + 0.5));
...
static void lcd_implementation_status_screen() {
  ...
  bool blink = lcd_blink();
    ...
    if (current_block->laser_status == LASER_ON) {
      u8g.drawBitmapP(5,13, ICON_BYTEWIDTH, ICON_HEIGHT, laseron_bmp);
      u8g.print(itostr3(current_block->laser_intensity));
      lcd_printPGM(PSTR("%"));
    } else {
      u8g.drawBitmapP(5,13, ICON_BYTEWIDTH, ICON_HEIGHT, laseroff_bmp);
      lcd_printPGM(PSTR("---%"));
    }
    ...
    u8g.drawBitmapP(39, 13, 3, 18,
      #if HAS(FAN)
        blink && fanSpeed ? info_fan_0 : info_fan_1
      #else
        info_fan_0
      #endif
    );
    if (blink && isHeatingHotend(0)) {
      u8g.drawBitmapP(67, 19, 3, 12,info_hotend_1);
    }
    else {
      u8g.drawBitmapP(67, 19, 3, 12,info_hotend_0);
    }
    if(blink && isHeatingBed()) {
      u8g.drawBitmapP(98, 19, 3, 12,info_bed_1);
    }
    else {
      u8g.drawBitmapP(98, 19, 3, 12,info_bed_0);
    }
  ...
  lcd_setFont(FONT_STATUSMENU);
    ...
    */
    // Progress bar frame
    u8g.drawFrame(1, 41, 126, 4 - (TALL_FONT_CORRECTION));
    //u8g.drawBox(2, 42, (unsigned int)(124.f * 100 / 100.f), 2 - (TALL_FONT_CORRECTION));
    // SD Card Progress bar and clock
    if (IS_SD_PRINTING) {

      //u8g.drawBox(2, 42, (unsigned int)(71.f * card.percentDone() / 100.f), 2 - (TALL_FONT_CORRECTION));
      u8g.drawBox(2, 42, (unsigned int)(124.f * card.percentDone() / 100.f), 2 - (TALL_FONT_CORRECTION));
      u8g.setPrintPos(2, 54);
      lcd_print('P');
      lcd_print(itostr3(int(card.percentDone()) + 0.5 ));
      lcd_print('%');
    }else{
      u8g.setPrintPos(2, 54);
      lcd_print("P---%");
    ...
    u8g.setPrintPos(53, 54);
    //el tiempo que lleva imprimiendo
    millis_t time = print_job_counter.duration() / 60;
    //SERIAL_EMV("time:", time);
    //SERIAL_EMV("print_job_counter:", millis());
    //timer esta en minutos
    if (time != 0) {
      ...
      #else
      //imprime el tiempo de imprecion
        ...
        u8g.setPrintPos(90, 54);
        /*
        if(last_time != time){
          //guarda el ultimo momento de tiempo
          last_time = time
        }
        */
  ...
  // Fan
  u8g.setPrintPos(37, 39);
  #if HAS(FAN)
    int per = ((fanSpeed) * 100) / 255;
  ...
  if(ver_cordenadas){
    ...
    //65.535 el numero limite que se puede mostrar (solo se puede llegar a 5.000 naturalmente)
    int capa_val = 16;
    u8g.setPrintPos(2, XYZ_BASELINE);
    lcd_printPGM(PSTR(MSG_CAPAS));

    if(actual_capas < 10){
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(actual_capas < 100){
      capa_val = 16 + 6;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(actual_capas < 1000){
      capa_val = 16 + 12;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(actual_capas < 10000){
      capa_val = 16 + 18;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else{
      capa_val = 16 + 24;
    ...
    lcd_print(ftostr5rj(actual_capas));
    lcd_print('/');
    //capa_val = capa_val + 6;
    if(total_capas < 10){
      capa_val = capa_val + 12;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(total_capas < 100){
      capa_val = capa_val + 18;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(total_capas < 1000){
      capa_val = capa_val + 24;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else if(total_capas < 10000){
      capa_val = capa_val + 30;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }else{
      capa_val = capa_val + 36;
      u8g.setPrintPos(capa_val, XYZ_BASELINE);
    }
    lcd_print(ftostr5rj(total_capas));
  }
  u8g.setColorIndex(1); // black on white
  ...
  lcd_print('%');
  u8g.drawBitmapP(8, 18, 3, 13, info_velo);

  //u8g.drawLine(64, 9, 64, 40);
  ...
  #else
  //letreto de info
    u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, 9);
    u8g.setPrintPos(1, 8);
    u8g.setColorIndex(0);
    lcd_print(lcd_status_message);
    u8g.setColorIndex(1);
